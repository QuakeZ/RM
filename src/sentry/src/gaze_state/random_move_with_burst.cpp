...
float power;//功率
float energy = 200;//缓冲能量 energy = energy-(power-40)*T,T为检测周期
...
int STart = 1;//开关：是否启动随机运动模式
int burst = 1;//开关：是否进入暴走模式
float burst_x = 1;//设置暴走模式下的增大倍率，这里需要调整
float energy_gate = 0.7;//暴走模式的期望速度缩减比率

void poseCallback(const  nav_msgs::OdometryConstPtr& odom)
{   
  float test = 0;
  test = odom->pose.pose.position.x-0.17;
...
void Fist_Set_Goal()		//使循环函数能够成功循环，因为在仿真中，在前几次接收消息的时候，机器人的位置和速度都为0，导致目标一直不达成，最终程序卡死
{
	if (!fist_goal_set)
	{
		...
		if (0 < robot.pose_x && robot.pose_x < 2.2)//这里我假定，当传感器节点数据异常或者接收不到的时候，这里的robot.pose_x为0;所以，只有当这个数据正常的时候才开始运行节点，即进入第一次循环。我认为传感器的异常信息应该交由传感器信息发布节点来排查。
		{       
			whole_goal_speed();
			fist_goal_set = true;
		}
...
{	
	...
	if (STart == 1)//判断是否发布信息的条件,若不满足，则不发送消息，即该模式不起用
	   ...
	   {	
		...
		else commandTurtle(vel_pub,set_speed);
   	   }	
	   else if(burst == 0)
	   {
		commandTurtle(vel_pub,set_speed);
	   }
	//ROS_INFO(" %f, %f, %f, %f",robot.pose_x,robot.speed,set_speed,Goal_x);//robot.speed
        };
...
int main(int argc, char *argv[])
{
 ...
 ros::NodeHandle nh;
 ros::Subscriber pose_sub = nh.subscribe("/chassis/odom", 1, poseCallback);
 ros::Publisher vel_pub = nh.advertise<geometry_msgs::Twist>("/chassis/cmd_vel", 10);//geometry_msgs::Twist这个就是发布的消息类型，若自定义，可以改为 功能包名::自定义消息文件名称。10是缓存区，表示最多储存的消息内容的数量。
