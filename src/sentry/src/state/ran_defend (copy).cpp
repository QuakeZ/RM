...
{
  robot.pose_x = pose->x - 5.544445;
  //robot.speed = pose->linear_velocity;
}
...
bool out_power_2()  //设置功率阈值二，赛规虽然有缓冲能量200J，但上限功率为30W
{
	return (power - 30) < 0;
}
//*******************************判断行*************************//
...
{								//又因为会减速，所以我想在达到目标的判定条件中加一个大一些的区间，作为一个刹车区间。
	...
	right_side = 1.8 - robot.pose_x;//轨道的长度为2米，为了防止超轨道，先设置为1.8M
...
{	
	...
	if (STart == 2)
        {
           commandTurtle(vel_pub,robot.speed);//发布处。
		   ROS_INFO("publish linear_x :%f",robot.speed);
        };
...
int main(int argc, char *argv[])
{
 ...
 ros::NodeHandle nh;
 ros::Subscriber pose_sub = nh.subscribe("/sentry/position", 1, poseCallback);//消息名称要改
 //ros::Subscriber pose_sub = nh.subscribe("robot_pose", 1, poseCallback);//消息名称要改
 ros::Publisher vel_pub = nh.advertise<geometry_msgs::Twist>("chassis_cmd_vel", 1);//geometry_msgs::Twist这个就是发布的消息类型，若自定义，可以改为 功能包名::自定义消息文件名称。10是缓存区，表示最多储存的消息内容的数量。
