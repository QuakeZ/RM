...
MatrixXd Z(2,1),R(2,2),U(1,1),K(2,2),Y(1,1);//测量量矩阵，状态更新的参数，控制量矩阵（加速度），卡尔曼系数，输出矩阵
MatrixXd F(2,2),G(2,1),H(2,2);//F!G!H!协方差外推
MatrixXd Q(2,2),W(2,1),V(2,2);//协方差外推的噪声，状态外推的噪声
...
//temp
    R << 10000,0,0,10000;//测量不确定度
    Q << 20,0,0,20;//过程噪声方差
    

////////////////////////////////////////////////////////////////////////

    ros::init(argc,argv,"positioningx");
    ros::NodeHandle nh;
    ros::Rate rate(50);

    ros::Subscriber t_sub = nh.subscribe("/sentry/fake/twist", 10, velCallback);
    ros::Subscriber p_sub = nh.subscribe("/sentry/fake/pose", 10, posCallback);
    ros::Publisher po_pub = nh.advertise <geometry_msgs::Pose>("/sentry/positionx", 1000);
