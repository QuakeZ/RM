...
////////////////////////////////////////////////////////////////////////
    ...
    ros::init(argc,argv,"positioningv");
    ros::NodeHandle nn;
    ros::Rate rate(50);

    ros::Subscriber t_sub = nn.subscribe("/sentry/fake/twist", 10, velCallback);
    ros::Subscriber p_sub = nn.subscribe("/sentry/fake/pose", 10, posCallback);
    ros::Publisher po_pub = nn.advertise <geometry_msgs::Pose>("/sentry/positionv", 1000);
    ...
    while(ros::ok())
    {
	    ...
	    else
	    {
            ...
            * ( MatrixXd::Identity(2,2) - K * H ).transpose()
            ...
            msgp.position.z = posr-y;//误差值 posr:real pos
